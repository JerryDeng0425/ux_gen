...
} catch { $checks.outputWritable = $false }

$passed = $checks.packageJson -and $checks.node -and $checks.dependencies -and $checks.browserAvailable -and $checks.outputWritable
[ordered]@{ passed = $passed; checks = $checks } | ConvertTo-Json -Depth 5
if (-not $passed) {
  Write-Error "Preflight failed. Install Node 20+, run npm install, provide Chromium/system Chrome, and grant write access to the selected output directory."
  exit 2
}
